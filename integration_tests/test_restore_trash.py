...
class describe_restore_trash:
    ...
    def it_should_error_when_user_input_is_not_a_number(self):
        ...
        self.when_running_restore_trash(from_dir='/foo', with_user_typing='-@notanumber')
        self.error_should_be('Invalid entry\n')

    @istest
    def it_should_error_when_user_input_is_too_small(self):
        self.having_a_trashed_file('/foo/bar')
        self.when_running_restore_trash(from_dir='/foo', with_user_typing='-1')
        self.error_should_be('Invalid entry\n')
    ...
    def it_should_show_the_file_deleted_from_the_current_dir(self):
        ...
        self.output_should_match('   0 2000-01-01 00:00:01 /foo/bar\n.*\n')
    ...
    def it_should_exit_gracefully_when_user_selects_nothing(self):
        self.having_a_trashed_file('/foo/bar')
        self.when_running_restore_trash(from_dir='/foo', with_user_typing='')
    ...
    def it_should_refuse_overwriting_existing_file(self):
        ...
        open('foo', 'a+').close()
        os.chmod('foo', 000)
        self.when_running_restore_trash(from_dir=current_dir(), with_user_typing='0')
    ...
    def setUp(self):
        ...
        out = OutputCollector()
        err = OutputCollector()
        self.when_running_restore_trash = RestoreTrashRunner(out, err, 'XDG_DATA_HOME')
...
class TrashCan:
    ...
    def write_trashed_file(self, path):
        ...
        write_file(f'{self.path}/info/foo.trashinfo', a_trashinfo(path))
        write_file(f'{self.path}/files/foo')
