...
from nose.tools import assert_equals, assert_regexp_matches, assert_true, istest

from trashcli.cmds import empty
from trashcli.empty import EmptyCmd
from trashcli.fs import FileRemover, mkdirs
from trashcli.trash import FileSystemReader
from nose.tools import assert_count_equal as assert_items_equal
from .files import having_file, make_dirs, require_empty_dir, set_sticky_bit, write_file
...
class TestTrashEmptyCmd:
    ...
    def test_trash_empty_will_crash_on_unreadable_directory_issue_48(self):
        ...
        empty(['trash-empty'], stdout=out, stderr=err, environ={'XDG_DATA_HOME': 'data'})
...
class When_invoked_with_N_days_as_argument:
    ...
    def having_a_trashed_file(self, name, date):
        contents = f'DeletionDate={date}T00:00:00\n'
        write_file(self.trashinfo(name), contents)

    def trashinfo(self, name):
        return f'{self.xdg_data_home}/Trash/info/{name}.trashinfo'
